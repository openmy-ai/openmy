...
class CleanTextCallsGeminiAPITest(unittest.TestCase):
    """验证 clean_text 会调用 Gemini API"""

    @patch('openmy.services.cleaning.cleaner.clean_with_gemini_api')
    def test_clean_text_delegates_to_gemini(self, mock_gemini):
        mock_gemini.return_value = "清洗后的文本"
        result = clean.clean_text("原始文本")
        mock_gemini.assert_called_once_with("原始文本", api_key=None)
...
class GeminiAPICallTest(unittest.TestCase):
    """验证 Gemini API 调用的参数和错误处理"""

    @patch('openmy.services.cleaning.cleaner.genai')
    def test_successful_call(self, mock_genai):
        mock_client = MagicMock()
        mock_genai.Client.return_value = mock_client
        mock_response = MagicMock()
        mock_response.text = "  清洗后文本  "
        mock_client.models.generate_content.return_value = mock_response

        result = clean.clean_with_gemini_api("原始文本", api_key="test-key")
        self.assertEqual(result, "清洗后文本")

        # 验证调用了正确的模型
        call_args = mock_client.models.generate_content.call_args
        self.assertEqual(call_args.kwargs.get('model') or call_args[1].get('model'), clean.GEMINI_MODEL)
    ...
    def test_empty_response_raises(self, mock_genai):
        ...
        with self.assertRaises(RuntimeError) as ctx:
            clean.clean_with_gemini_api("原始文本", api_key="test-key")
        self.assertIn("没有返回内容", str(ctx.exception))

    def test_missing_api_key_raises(self):
        with patch.dict('os.environ', {}, clear=True):
            with self.assertRaises(RuntimeError) as ctx:
                clean.clean_with_gemini_api("原始文本")
...
class TimeHeaderValidationTest(unittest.TestCase):
    ...
    def test_clean_text_full_pipeline(self, mock_corrections, mock_gemini):
        """验证 clean_text 完整流程：Gemini API + 校验 + 纠错"""
