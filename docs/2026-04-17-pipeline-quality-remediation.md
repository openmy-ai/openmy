...
Codex 对 2026-04-15 和 2026-04-16 两天数据执行了完整 pipeline。产出数据通过人工审查发现 7 类严重问题。
...
**现象**：4/15、4/16 的事件顺序和时段排列错误。

**根因**：多段录音在 STT 阶段被合成为一个 batch，真实开始时间被抹掉。下游场景切分和 time_block 排列依赖这些时间戳，导致整天的事件顺序失真。

**状态**：⚠️ **未修复**。本次修复集中在后半段 pipeline（质量检测、蒸馏、日报生成），转写层的时间轴保真问题需要单独处理，涉及 `audio_pipeline.py` 和 `segmenter.py` 的时间戳传递逻辑。

**影响**：time_blocks 的时段划分可能不准确，但 work_sessions、scene 内容和结构化回灌不受影响。
