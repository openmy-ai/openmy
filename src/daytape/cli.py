...
def _load_context_snapshot():
    ...
    if not ctx_path.exists():
        console.print("[red]❌ 还没有活动上下文快照，请先运行 `python3 -m daytape context`[/red]")
        return None
...
def _append_context_correction(
    ...
    from daytape.services.context.corrections import append_correction, create_correction_event
...
def cmd_correct_list(_args: argparse.Namespace) -> int:
    """查看 correction 历史。"""
    from daytape.services.context.corrections import load_corrections
...
def cmd_correct(args: argparse.Namespace) -> int:
    ...
    if not tokens:
        console.print("[yellow]用法：daytape correct <date> <wrong> <right> 或 daytape correct <action> ...[/yellow]")
        return 1
    ...
    if action == "typo":
        if len(tokens) != 4:
            console.print("[red]❌ typo 用法：daytape correct typo <date> <wrong> <right>[/red]")
            return 1
    ...
    if action == "close-loop":
        if len(tokens) != 2:
            console.print("[red]❌ close-loop 用法：daytape correct close-loop <title> [--status done|abandoned][/red]")
            return 1
        loop = _resolve_item(
        ...
        console.print("[dim]运行 `python3 -m daytape context` 重新生成快照[/dim]")
        return 0

    if action == "reject-loop":
        if len(tokens) != 2:
            console.print("[red]❌ reject-loop 用法：daytape correct reject-loop <title>[/red]")
            return 1
        loop = _resolve_item(
            ctx.rolling_context.open_loops,
            tokens[1],
    ...
    if action == "merge-project":
        if len(tokens) != 3:
            console.print("[red]❌ merge-project 用法：daytape correct merge-project <from> <into>[/red]")
            return 1
    ...
    if action == "reject-project":
        if len(tokens) != 2:
            console.print("[red]❌ reject-project 用法：daytape correct reject-project <title>[/red]")
            return 1
    ...
    if action == "reject-decision":
        if len(tokens) != 2:
            console.print("[red]❌ reject-decision 用法：daytape correct reject-decision <text>[/red]")
            return 1
