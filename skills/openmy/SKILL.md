...
| `openmy run YYYY-MM-DD --audio file.m4a` | 全流程：去静音→压缩→切块→转写→清洗→场景切分→蒸馏→提取→日报 |
...
| `openmy skill status.get --json` | 稳定 JSON 状态入口 |
| `openmy skill day.get --date YYYY-MM-DD --json` | 稳定 JSON 单日入口 |
| `openmy skill context.get --json` | 稳定 JSON 上下文入口 |
...
| 蒸馏/提取失败 | 确认 `OPENMY_LLM_API_KEY` 或 `GEMINI_API_KEY` |
