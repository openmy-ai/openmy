...
function formatPlaybackClock(value) {
...
function ensureSceneAudio() {
  ...
  sharedSceneAudio = new Audio();
  sharedSceneAudio.preload = 'metadata';
  sharedSceneAudio.addEventListener('timeupdate', () => {
    if (!state.playback.activeKey) return;
    const current = Number(sharedSceneAudio.currentTime || 0);
    if (current >= state.playback.sceneEnd - 0.05) {
      state.playback.progress = state.playback.duration;
      if (!sharedSceneAudio.paused) {
        sharedSceneAudio.pause();
      }
      sharedSceneAudio.currentTime = state.playback.sceneEnd;
    } else {
      state.playback.progress = Math.max(0, current - state.playback.sceneStart);
    }
    syncScenePlaybackUi();
  });
  sharedSceneAudio.addEventListener('play', () => {
    state.playback.status = 'playing';
    syncScenePlaybackUi();
  });
  sharedSceneAudio.addEventListener('pause', () => {
    if (!state.playback.activeKey) return;
    state.playback.status = 'paused';
    syncScenePlaybackUi();
  });
  sharedSceneAudio.addEventListener('ended', () => {
    state.playback.status = 'paused';
    state.playback.progress = state.playback.duration;
    syncScenePlaybackUi();
  });
...
async function loadSceneAudioSource(audio, url) {
...
async function prepareScenePlayback(target) {
...
function renderScenePlaybackControl(segment) {
...
async function toggleScenePlayback(segmentTime) {
...
async function seekScenePlayback(segmentTime, rawValue) {
  const target = getSegmentSceneTarget(segmentTime);
...
function setScenePlaybackRate(_segmentTime, rawValue) {
